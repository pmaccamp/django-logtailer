...
LogTailer.getHistory = function (callback) {
    LogTailer.currentScrollPosition = django.jQuery("#log-window").scrollTop();
    django.jQuery.ajax({
        url: LOGTAILER_URL_GETHISTORY + django.jQuery("#history-lines").val() + "/",
        success: function (result) {
            LogTailer.printLines(result);
            callback && callback();
        },
...
LogTailer.startReading = function () {
    if (django.jQuery('#log-window').html() === "") {
...
LogTailer.clearLog = function () {
    console.log(django.jQuery('#log-window'))
